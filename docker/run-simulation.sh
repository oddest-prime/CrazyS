...
echo "insert into runs values('${date_hash_mode}', '${hash}', '${mode}', '${params}', '${obstacleScenario}', '${pathScenario}', '${extra}', '${launch_file}');" | sqlite3 -cmd ".timeout 10000" /crazyflie_ws/src/crazys/simulations.database 2> /tmp/db.error
...
do
    ...
    echo "insert into metrics values('${date_hash_mode}', '${field_name}', '${field_val}');" | sqlite3 -cmd ".timeout 10000" /crazyflie_ws/src/crazys/simulations.database 2> /tmp/db.error
done
...
do
    ...
    then
        ...
        field_val="$line3"
        echo "insert into params values('${date_hash_mode}', '${field_name}', '${field_val}');" | sqlite3 -cmd ".timeout 10000" /crazyflie_ws/src/crazys/simulations.database 2> /tmp/db.error
    fi;
done;
IFS=$SAVEIFS

cp /tmp/video.info /tmp/log_output/simulation.info
cp /tmp/db.error /tmp/log_output/db.error
