...
docker run --rm --volume ~/SWARM/crazys:/crazyflie_ws/src/crazys crazys /crazyflie_ws/src/crazys/docker/run-simulation.sh `git rev-parse --short HEAD` dist2 dist mpc1_params2a 0 4 "eel" &
sleep 150 # delay compilation by 150 seconds in second two docker containers
wait
...
docker run --rm --volume ~/SWARM/crazys:/crazyflie_ws/src/crazys crazys /crazyflie_ws/src/crazys/docker/run-simulation.sh `git rev-parse --short HEAD` dist2 dist mpc1_params2b 0 4 "eel" &
sleep 150 # delay compilation by 150 seconds in second two docker containers
wait
test -e /tmp/stop && exit 7 # stop if flag is present
...
docker run --rm --volume ~/SWARM/crazys:/crazyflie_ws/src/crazys crazys /crazyflie_ws/src/crazys/docker/run-simulation.sh `git rev-parse --short HEAD` dist2 dist mpc1_params2c 0 4 "eel" &
sleep 150 # delay compilation by 150 seconds in second two docker containers
wait
test -e /tmp/stop && exit 7 # stop if flag is present
